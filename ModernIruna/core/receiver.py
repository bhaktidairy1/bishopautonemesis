...
def handle_0130_char_stats(payload: bytes):
    ...
    try:
        ...
        if state.char_name:
            ...
            name_indices = [m.start() for m in re.finditer(name_hex, data_hex)]
            for n_idx in name_indices:
                if n_idx >= 28 and n_idx % 2 == 0:
                    spina_h = data_hex[n_idx-28:n_idx-20]
                    hp_h = data_hex[n_idx-20:n_idx-12]
                    mp_h = data_hex[n_idx-12:n_idx-4]
                    nlen_h = data_hex[n_idx-4:n_idx]
                    try:
                        hp = int(hp_h, 16)
                        mp = int(mp_h, 16)
                        nlen = int(nlen_h, 16)
        ...
        if final_spina is not None:
            ...
            print(f"[*] Login Stats Loaded: Spina: {final_spina:,} | HP {state.player_hp}/{max_hp_math} | MP {state.player_mp}/{max_mp_math}")
            return
    except Exception as e:
