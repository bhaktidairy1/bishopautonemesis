...
def teleport(sock, map_id: int, x: int = None, y: int = None):
    ...
    # Clamp coordinates to valid range and cast to int
    x = int(max(0, min(156, x)))
    y = int(max(0, min(156, y)))
    ...
    try:
        ...
        print("[-] Teleport timeout (b503). Server unresponsive. Aborting teleport.")
        state.paused = was_paused
        return {"status": "timeout"}
